...
class ManitoulinAdapter extends CarrierAdapter {
  ...
  _getMockRates(params) {
    ...
    const services = { envelope: ['Express Courier', 'Standard Courier', 'Economy'], parcel: ['Parcel Express', 'Ground', 'Economy Ground'], ltl: ['Direct Freight', 'Consolidated LTL', 'Economy LTL'] };
  ...
  async bookShipment(details) {
    return { carrierTrackingNumber: `MAN-${new Date().getFullYear()}-${String(Date.now()).slice(-5)}`, confirmationNumber: `MAN-CONF-${Date.now()}`, status: 'confirmed' };
  }
}
