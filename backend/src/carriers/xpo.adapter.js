...
class XPOAdapter extends CarrierAdapter {
  ...
  _getMockRates(params) {
    ...
    const services = { envelope: ['Express', 'Priority', 'Standard'], parcel: ['Ground', 'Express', '2-Day'], ltl: ['LTL Standard', 'LTL Priority', 'LTL Economy'] };
  ...
  async bookShipment(details) {
    return { carrierTrackingNumber: `XPO${Date.now()}`, confirmationNumber: `XPO-CONF-${Date.now()}`, status: 'confirmed' };
  }
}
